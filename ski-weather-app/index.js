...
app.post("/weather", async (req, res) => {
  ...
  const displayQuery =
    resort.toLowerCase().includes("ski") ||
  ...
  try {
    ...
    if (USE_MOCK_DATA) {
      ...
      for (const item of forecastList) {
        ...
        if (!dayMap[day]) {
          ...
          dayMap[day].temp.min = Math.min(
            dayMap[day].temp.min,
            item.main.temp_min,
          );
          dayMap[day].temp.max = Math.max(
            dayMap[day].temp.max,
            item.main.temp_max,
          );
      ...
      weatherData = {
        ...
        current: {
          ...
          rain: currentWeatherResponse.data.rain
            ? currentWeatherResponse.data.rain["1h"]
            : 0,
          snow: currentWeatherResponse.data.snow
            ? currentWeatherResponse.data.snow["1h"]
            : 0,
        },
        daily: dailyData,
      };
    ...
    try {
      ...
      aiAnalysis = await analyzeResortConditions(
        resort,
        weatherData,
        nearbyResorts,
      );
    ...
    if (error.response) {
      if (error.response.status === 401) {
        errorMessage =
          "Invalid API key. Please check your OpenWeatherMap API key in the .env file.";
...
const USE_MOCK_DATA =
  !WEATHER_API_KEY || WEATHER_API_KEY === "your_openweathermap_api_key_here";

if (USE_MOCK_DATA) {
  console.warn(
    "\x1b[33m%s\x1b[0m",
    "Warning: OpenWeatherMap API key is not set or is using the default value.",
  );
  console.warn(
    "\x1b[33m%s\x1b[0m",
    "Please set a valid API key in the .env file or as an environment variable.",
  );
  console.warn(
    "\x1b[33m%s\x1b[0m",
...
if (require.main === module) {
  app.listen(PORT, "0.0.0.0", () => {
    ...
    console.log(
      `Also accessible at https://work-1-ilsejzrsbsscbydv.prod-runtime.all-hands.dev`,
    );
  });
}
