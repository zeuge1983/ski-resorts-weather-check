...
describe('Snow Analyzer', () => {
  describe('analyzeSnowConditions', () => {
    test('should correctly identify multiple snow days', () => {
      ...
      expect(result.analysis).toBe('Great news! Fresh powder expected with multiple snow days in the forecast.');
    });
    ...
    test('should correctly identify a single snow day', () => {
      ...
      expect(result.analysis).toBe('Some fresh snow expected with 1 snow day(s) in the forecast.');
    });
    ...
    test('should correctly identify cold conditions with no snow', () => {
      ...
      expect(result.analysis).toBe('No fresh snow expected, but temperatures will be cold enough to preserve existing snow.');
    });
    ...
    test('should correctly identify cool conditions with no snow', () => {
      ...
      expect(result.analysis).toBe('No fresh snow expected. Cool temperatures may help maintain snow conditions.');
    });
    ...
    test('should correctly identify warm conditions with no snow', () => {
      ...
      expect(result.analysis).toBe('No fresh snow expected and temperatures are relatively warm. Snow conditions may deteriorate.');
    });
